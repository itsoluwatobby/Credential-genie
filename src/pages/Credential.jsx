  ...
  return (
    <main className="hidebars flex flex-col gap-y-5 p-4 w-full h-full">
      ...
      <input
        ...
        className="border h-12 p-1"
        onChange={(e) =>
          setUserDetail((prev) => ({ ...prev, recipientDID: e.target.value }))
        }
      />
      <input
        type="email"
      ...
      <div className="flex items-center w-[80%] h-12 gap-1">
        <input
          type="text"
          value={key}
          className="border h-full w-full p-1"
          placeholder="name"
          onChange={(e) =>
        ...
        {credentials.vcs.map((cred, index) => (
          <div key={index} className="flex flex-col gap-y-1">
            <p className="whitespace-nowrap font-bold">
              Title:{' '}
              <span className="whitespace-pre-wrap break-all font-normal">
                {cred.title?.substring(0, 150)}...
              </span>
            </p>
